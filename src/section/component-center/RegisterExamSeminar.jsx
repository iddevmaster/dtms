...
export default class RegisterExamSeminar extends Component {
  state = {
  ...
  refreshDataExamDate = async () => {
    try {
      await axios
        .get(
          Common.API_URL +
            `course/examdate/${this.state.school_id}?branch_id=${this.state.branch_id}&vehicle_type_id=${this.state.vehicle_type_id}&month=${this.state.present_month2}&year=${this.state.present_year2}`,
  ...
  setMonthFilter2 = async (e) => {
    ...
    await new Promise((accept) =>
      this.setState(
        {
          present_month2: getmonth,
          present_year2: getyear,
        },
        accept
      )
    );
  ...
  handleUpdateExamDate = () => {
    try {
      axios.get(
        Common.API_URL +
          `register/update_exam_tmp/${rm_id}/${this.state.ed_id}`,
        Common.options
      );
    } catch (error) {
      console.log(error);
    ...
    return (
      <div>
        ...
        <Row>
          <Col>
            <Card border="success">
              <Card.Body style={{ height: "650px", overflow: "scroll" }}>
                <Card.Title>
                  <Row>
                    ...
                    <Col>
                      <div align="right">
                        <DatePicker
                          ...
                          locale={thai_th}
                          style={{ height: "45px" }}
                          onChange={(e) => this.setMonthFilter2(e)}
                          placeholder={c}
                        />
                      </div>
                    </Col>
        ...
        <div className="d-grid gap-2">
          <p></p>
          <Button
            disabled={setPostSeminar.length > 0 && ed_id > 0 ? false : true}
            variant="primary"
            size="lg"
            onClick={this.submit}
          >
